...
@section('content')
    ...
    @if (session('error'))
        <div id="toastError"
            class="fixed top-24 left-1/2 -translate-x-1/2 z-50 bg-red-500 text-white px-6 py-3 rounded-xl shadow-lg flex items-center gap-3 transition-all duration-500">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    {{-- INFO batas pinjam sudah tercapai --}}
    @if ($jumlahAktif >= 3)
        <div class="mb-5 bg-yellow-50 border border-yellow-200 text-yellow-700 px-5 py-4 rounded-xl flex items-start gap-3">
            <svg class="w-5 h-5 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M12 9v2m0 4h.01M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z" />
            </svg>
            <div>
                <p class="font-semibold text-sm">Batas Peminjaman Tercapai</p>
                <p class="text-sm mt-0.5">Kamu sudah meminjam 3 buku. Kembalikan buku terlebih dahulu sebelum meminjam lagi.
                </p>
            </div>
        </div>
    @endif
        ...
        @foreach ($buku as $b)
            <div class="bg-white rounded-xl shadow-md flex overflow-hidden">
                ...
                <div class="p-4 pt-6 flex flex-col flex-1">
                    <div>
                        <h3 class="font-semibold text-lg text-[#5C7F9C]">{{ $b->judul }}</h3>
                        <p class="text-sm text-gray-600">Kategori : {{ $b->kategori->nama_kategori ?? '-' }}</p>
                    ...
                    <div class="flex gap-2 mt-3">
                        ...
                        @elseif ($jumlahAktif >= 3)
                            <span
                                class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-400 gap-1 cursor-not-allowed"
                                title="Kamu sudah meminjam 3 buku">
                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <div id="modalPinjam" class="hidden fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
        <div class="bg-white w-96 p-6 rounded-2xl shadow-lg">
            <h2 class="text-lg font-semibold text-[#5C7F9C] mb-2 text-center">Pinjam Buku</h2>
